<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl">Edit Employee</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('employees.update', $employee) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="first_name" class="w-full border p-2" required value="{{ old('first_name', $employee->first_name) }}">
                    @error('first_name') <span class="text-red-600">{{ $message }}</span> @enderror
                </div>

                <div class="mb-4">
                    <label class="block font-semibold">Last Name *</label>
                    <input type="text" name="last_name" class="w-full border p-2" required value="{{ old('last_name', $employee->last_name) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" class="w-full border p-2" value="{{ old('email', $employee->email) }}">
                    @error('email') <span class="text-red-600">{{ $message }}</span> @enderror
                </div>

                <div class="mb-4">
                    <label class="block font-semibold">Phone</label>
                    <input type="text" name="phone" class="w-full border p-2" value="{{ old('phone', $employee->phone) }}">
